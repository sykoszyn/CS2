...
export default async function FavoritesPage() {
  ...
  return (
    <div className="px-4 py-8 lg:px-6">
      ...
      {isEmpty ? (
        <div className="mt-8">
          <EmptyState icon={Heart} title={t("emptyTitle")} description={t("emptyDescription")} />
        </div>
      ) : (
        <div className="mt-8 space-y-10">
          {lineups.length > 0 && (
            <section>
              <h2 className="mb-3 font-display text-lg font-semibold">
                {t("sections.lineups", { count: lineups.length })}
              </h2>
          ...
          {boosts.length > 0 && (
            <section>
              <h2 className="mb-3 font-display text-lg font-semibold">
                {t("sections.boosts", { count: boosts.length })}
              </h2>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {boosts.map((boost, i) => (
          ...
          {plays.length > 0 && (
            <section>
              <h2 className="mb-3 font-display text-lg font-semibold">
                {t("sections.plays", { count: plays.length })}
              </h2>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {plays.map((play, i) => (
