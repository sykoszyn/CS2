...
export default async function BoostDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const t = await getTranslations("boosts.detail");
  const tCategory = await getTranslations("labels.boostCategory");
  const tNav = await getTranslations("nav");

  return (
    <div className="px-4 py-8 lg:px-6">
      <div className="flex items-center gap-2 text-sm text-foreground-muted">
        <Link href={`/maps/${boost.mapSlug}`} className="hover:text-brand">
          {map?.name ?? boost.mapSlug}
        </Link>
        <span>/</span>
        <span>{tNav("boosts")}</span>
      </div>
      <h1 className="mt-1 font-display text-2xl font-bold">{boost.name}</h1>
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <Badge variant="brand">{tCategory(boost.category)}</Badge>
        <Badge className="gap-1">
          <Users size={12} /> {t("playersRequired", { count: boost.playersRequired })}
        </Badge>
      ...
      {boost.authorUsername && (
        <Link href={`/profile/${boost.authorUsername}`} className="mt-2 inline-block text-sm text-foreground-muted hover:text-brand">
          {t("by", { username: boost.authorUsername })}
        </Link>
      )}
